...
const ManageSecurity: React.FC = () => {
    ...
    const [selectedWatchman, setSelectedWatchman] = useState<{ id: string; name: string } | null>(null);
    ...
    const filteredWatchmen = watchmen.filter(watchman => {
        const search = searchTerm.toLowerCase();
        return (
            watchman.name.toLowerCase().includes(search) ||
            watchman.email.toLowerCase().includes(search)
        );
    });
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await adminService.deleteWatchman(selectedWatchman.id);
    ...
    return (
        <AdminLayout title="Manage Security" activeMenu="security">
            ...
            <div className="admin-page-content">
                <div className="page-header">
                    <div>
                        ...
                        <p className="page-subtitle">Manage security personnel and access logs</p>
                    </div>
                    <div className="header-actions">
                        ...
                        <button className="btn-primary" onClick={handleAddNew}>
                            <span className="icon">+</span> Add Watchman
                        </button>
                    </div>
                </div>
                        ...
                        <div className="table-responsive">
                            <table className="modern-table">
                                ...
                                <tbody>
                                        ...
                                        <tr key={item._id}>
                                            ...
                                            <td>
                                                <div className="action-buttons">
                                                    <button className="btn-view" onClick={() => navigate(`/admin/security-details/${item._id}`)}>
                                                        View Details
                                                    </button>
                                                    <button className="btn-icon delete" onClick={() => handleDeleteClick(item._id, item.name)} title="Remove Watchman">
                                                        🗑️
                                                    </button>
                                                </div>
                                            </td>
                                    ...
                                    {filteredWatchmen.length === 0 && (
                                        <tr>
                                            <td colSpan={4} className="empty-state">
                                                {searchTerm ? "No watchmen found matching your search." : "No watchmen found. Click \"Add Watchman\" to register one."}
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                            ...
                            <div className="modal-body">
                                <div className="form-group">
                                    ...
                                    <input
                                        type="text"
                                        required
                                        placeholder="e.g. Michael Security"
            ...
            <DeleteConfirmationModal
                ...
                itemName={selectedWatchman?.name || 'Watchman'}
            ...
            <style>{`
                ...
                .user-avatar-placeholder { width: 40px; height: 40px; border-radius: 50%; background: #dcfce7; color: #15803d; display: flex; align-items: center; justify-content: center; font-weight: 600; border: 2px solid #bbf7d0; overflow: hidden; position: relative; }
